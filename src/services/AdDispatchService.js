...
class AdDispatchService {
    ...
    try {
      ...
      for (const ad of pendingAds) {
        try {
          ...
          console.log(`Ad ${ad.id} sent to ${sentCount} users`);
        } catch (error) {
          console.error(`Failed to send ad ${ad.id}:`, error.message);
          await Ad.markFailed(ad.id);
        }
      }
    } catch (error) {
      console.error('AdDispatchService.processScheduledAds error:', error);
    } finally {
      this.isRunning = false;
    }
  }
    ...
    try {
      const [users] = await connection.execute(
        `SELECT id, fcm_token, device_platform FROM users
         WHERE fcm_token IS NOT NULL AND fcm_token != '' AND id != ?`,
        [ad.user_id]
      );
        ...
        try {
          await this.pushService.sendAlert({
            ...
            extra_data: {
              ...
              ad_id: String(ad.id),
              ad_type: ad.ad_type,
              provider_name: ad.user_name || 'Prestador',
            },
          });
          sentCount++;
        } catch {
  ...
  async sendTargeted(ad) {
    ...
    if (categories.length === 0) {
      console.warn(`Ad ${ad.id}: no target categories, falling back to general send`);
      return this.sendToAll(ad);
    }

    const interestedUsers = await UserSearchCategory.getUsersInterestedInCategories(categories, ad.user_id);
    ...
    if (providerLat && providerLng) {
      targetUsers = interestedUsers.filter(u => {
        if (!u.latitude || !u.longitude) return true;
        const dist = haversineDistance(providerLat, providerLng, Number(u.latitude), Number(u.longitude));
  ...
  startCronJob(intervalMs = 60000) {
    console.log(`AdDispatchService: starting cron job (interval: ${intervalMs}ms)`);
